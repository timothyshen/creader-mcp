...
export function registerAITools(server) {
    ...
    server.tool("analyze_book", "Deep Guardian analysis on a chapter. Uses vector-retrieved context to find character, plot, timeline, and worldbuilding inconsistencies. Returns up to 10 GuardianIssues with severity, category, evidence, and suggestion.", {
    ...
    }, { readOnlyHint: true, openWorldHint: true }, async ({ bookId, chapterId, focusAreas }) => {
        try {
            const client = getClient();
            ...
            const issues = await client.post(`/api/books/${bookId}/guardian/analyze`, { chapterId, chapterContent, focusAreas });
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("proofread", "Publishing-standard proofread of a chapter. Returns punctuation, typo, grammar, and formatting issues with char-offset textPosition and suggestedFix for each issue.", {
        bookId: z.string().describe("Book ID"),
        ...
        try {
            ...
            const issues = await client.post(`/api/books/${bookId}/guardian/proofread`, { chapterId, chapterContent });
            return {
                content: [
                    {
                        type: "text",
