...
import type { SemanticRelation } from "../lib/types.js"

const entityTypes = ["character", "location", "event", "note"] as const
...
export function registerRelationTools(server: McpServer) {
  server.tool(
    ...
    async ({ bookId }) => {
      try {
        ...
        const items = await client.get<SemanticRelation[]>(
          `/api/books/${bookId}/semantic-relations`
        )
        if (!items.length) {
        ...
        const text = items
          .map(
            (r) =>
              `- [${r.sourceType}:${r.sourceId}] --${r.type}--> [${r.targetType}:${r.targetId}]` +
  ...
  server.tool(
    ...
    async ({ bookId, sourceId, sourceType, targetId, targetType, type, inverseType, description, strength }) => {
      try {
        const client = getClient()
        const rel = await client.post<SemanticRelation>(
          `/api/books/${bookId}/semantic-relations`,
          { sourceId, sourceType, targetId, targetType, type, inverseType, description, strength }
        )
        return {
          content: [
            {
  ...
  server.tool(
    ...
    async ({ id, ...fields }) => {
      try {
        const client = getClient()
        const rel = await client.patch<SemanticRelation>(`/api/semantic-relations/${id}`, fields)
        return {
          content: [
            {
              type: "text" as const,
